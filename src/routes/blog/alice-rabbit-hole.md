...
<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>최근에 학습하면서 무슨 말인지 모르겠었던 개념은 무엇이 있나요?</li>
    <li>처음에는 낯설게 느껴졌는데, 지금은 당연한 것처럼 느껴지는 건 무엇이 있나요?</li>
    <li>오래 봐왔는데도, 여전히 낯설게 느껴지는 개념은 무엇이 있나요?</li>
  </ul>
</div>
...
`node-v`를 터미널에 쳤는데, 다음과 같은 에러가 나왔다고 해봅시다.
```bash
node-v
...
- `command`가 무엇인지?
- `command not found`는 무슨 뜻인지?
- 터미널은 command를 어떻게 찾아오는지?
- 명령어는 어떤 구조로 되어 있는지?
  - 예를 들어 option이나 flag는 무엇인지?

터미널에 익숙한 사람들은 이 에러의 원인을 빠르게 추측할 수 있고, 문제도 해결할 수 있습니다.

하지만 초보자들은 그런 경험이 없습니다. 상상의 나래를 펼치게 됩니다. 예를 들자면 다음과 같습니다.

- 소문자라 그런 건 아닐까? `-V`라고 치면 될 것 같아.
- `-`를 하나만 쳐서 그런 건 아닐까? `--` 두 번 치면 되지 않을까?
- 약어라서 그렇진 않을까? `--version`이라 치면 되지 않을까?
- 오타를 낸 것은 아닐까? `deno-v` 라고 치면 잘 되지 않을까?

중요한 사실은 이런 추측이 꼭 잘못되거나 이상한 게 아니라는 겁니다. 예를 들어 실제로 node의 창시자가 새로 만든 deno는 `deno -v`라고 치면 에러를 뱉습니다. `deno --version`이라고 쳐야 버전을 출력하죠.

정답은 `-v`와 같은 옵션은 `node` 같은 명령어(command)와 띄어 써주어야 한다는 것입니다. 그러니까 `node-v`처럼 붙여 쓰면 안 되고, `node -v`처럼 써줘야 한다는 것이죠. 붙여 쓰면 `node-v`를 통째로 하나의 명령어로 인식하게 됩니다.
...
<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>나중에 생각하면 어처구니 없는 미신이나 잘못된 생각으로 헤맨 적이 있나요?</li>
    <li>그때로 돌아간다면 어떻게 했으면 좋았을까요?</li>
  </ul>
</div>
...
문제는 불안이나 두려움이 강박으로 이어지는 겁니다. 앞서 말했듯 시니어가 되더라도 세상에는 모르는 게 훨씬 많을 겁니다. 불안을 해결하기 위해 

우리는 불안에 휘둘리는 게 아니라, 판단하고 결정을 해야 합니다. 무엇을 배우고, 무엇을 배우지 않을지. 내가 필요한 건 어디까지인지 말이죠.

<div class="admonition note">
  <p class="admonition-title">✏️ 비판적으로 생각해보기</p>
  <ul>
    <li>정말 그런가요? 몰라도 된다고 뒤로 밀어놨다가 뒤통수를 맞은 적이 있나요?</li>
    <li>이게 무엇이고 어떤 상황에 필요한지, 직접 써보지 않고 알려면 어떻게 해야할까요?</li>
  </ul>
</div>
...
- 그것의 존재조차 모른다
  - 예를 들어 커맨드라는 말을 처음 듣는다
- 들어봤지만, 모호하고 막연한 편견을 가지고 있다
  - 예를 들어 터미널은 대충 해커들이 쓰는 멋진 검은 화면이라 생각한다.
- 무엇이고 언제 왜 필요한지 대강 알지만, 할 줄은 모른다.
  - 예를 들어 명령어나 환경 변수에 대한 글을 북마크해놓고 안 읽고 있다
- 간단한 경험이 있지만, 자주 자료를 참고하거나 도움을 구해야 한다. 문제 해결에 시간이 오래 걸린다.
  - 예를 들어 책이나 강의를 보고 필요한 명령어를 따라칠 수 있다.
- 일반적인 경우는 잘 해낼 수 있지만, 특이한 경우는 도움을 구해야 한다. 상황에 맞지 않아도 익숙한 해결책을 적용하기도 한다.
  - 예를 들어 터미널에서 설치하고 실행하고, 환경 변수를 설정하는 일들을 할 수 있다. 책이나 문서가 낡거나 잘못되어 있으면 헤매게 된다.
- 빠르게 숙달되어서 할 수 있다. 자주 하는 실수를 알고 있고. 익숙하고 "유창(fluent)"하게 느껴진다. 특이하고 비전형적인 상황에도 대처할 수 있다.
  - 예를 들어 마우스를 쓰는 것보다 빠르게 터미널을 사용한다. 머리 속에 패키지가 설치되고 실행되는 심성 모형(mental model)을 그릴 수 있다.
...
예를 들어 저는...
- GraphQL이나 Relay는 왜 쓰는지 정도는 알지만, 뭘 만들어 본 적은 없습니다.
  - 배워보면 도움이 될만한 상황도 여러 번 봐서 공부해보려 해요.
- C나 Rust로 저수준 시스템 프로그래밍을 할 줄 모릅니다
  - 수업을 착각해서 논리회로 수업을 들은 적이 있습니다. 가산기나 이진수를 십진수로 변환하는 논리회로를 만들 줄 알지만, 쓸모가 없었습니다.
- ORM 없이 SQL로 10개가 넘는 테이블을 설계하거나, 최적화해본 적이 없습니다.
  - 앞으로도 ORM을 쓰거나 EdgeDb를 쓸 것 같아요.
- redux로 장난감 todo list 이상의 도메인을 모델링해본 적이 없습니다
  - 당분간은 redux-saga나 thunk 같은 미들웨어를 배울 생각이 없어요.
- css grid를 예제 복사 붙여넣기 이상으로 쓸 줄 모릅니다
  - CSS를 잘 몰라서 고통스러운 일이 많았습니다.

[추천하는 글](#추천하는-글)에 있는 댄 아브라모의 글도 읽어보세요.

<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>내가 알고 있는 것들을 쭉 적어보고, 수준에 따라 나눠보세요</li>
    <li>여기서 더 많이 알았으면 좋겠는 건 무엇인가요?</li>
    <li>열심히 배웠는데, 별로 쓸 때가 없었던 건 무엇인가요?</li>
  </ul>
</div>
...
- 리액트의 독특한 렌더링 방식을 잘 안다는 뜻인지?
- class component를 안다는 건지?
  - 아니면 구조 분해 할당과 hook을 잘 쓴다는 건지...
- suspense나 portal 같은 최신 기능을 잘 사용하는지?
- radix나 react-hook-form, react-query 같은 리액트 용 라이브러리를 잘 다루는지?
- nextjs나 remix 같은 서버 사이드 렌더링 프레임워크를 잘 다룬다는 건지?
- semantic html을 모르고, 모든 걸 div로 만든다면 리액트를 잘 하는 걸까요?
...
구체적인 목표를 세우는 법에 대해서도 [일일회고 가이드](https://twinstae.github.io/daily-review-guide/)에서 다룬 적 있습니다. 더 자세한 이야기가 필요하면 읽어보세요.

<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>나는 무엇을 잘 하고 싶나요? 그 목표는 "리액트를 잘 하고 싶다"처럼 막연한가요?</li>
    <li>목표를 작게 쪼개 보고, 어느 정도 수준으로 알고 싶은지 결정해보세요.</li>
  </ul>
</div>
...
이왕이면 구체적인 궁금증부터 시작해보세요. 예를 들어 저는 [리믹스를 쓰면서 tailwind를 설정](https://remix.run/docs/en/v1/guides/styling#tailwind-css)하는데, package.json에 다음과 같은 명령어를 넣으라고 하더군요.
...
```
그리고 [pino라는 logger](https://github.com/pinojs/pino-pretty#usage)를 설정하려 하니, 서버를 실행하는 명령어에 다음과 같이 넣어주라고 했습니다.
...
- `|`는 뭐하는 걸까요?
  - `node app.js` 뒤에 pino-pretty를 해주는 이유는 뭘까요?
- `run-p`는 어디에 쓰는 걸까요?
- 왜 `\"dev:*\""`처럼 `\"`라고 쓰는 걸까요?
- 저 옵션들은 무슨 뜻일까요?
- 왜 `--dir`은 `--`를 두 번 쓰는데, `-w`는 한 번만 쓸까요?
- `dev:css`랑 `dev:remix`는 각각 어떤 역할을 하는 걸까요?
...
<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>요즘 업무에서 귀찮거나 어려움을 느끼는데, 어쩔 수 없다 여기는 건 무엇이 있나요?</li>
    <li>지난 1주일 동안 공부한 게 있나요? 그걸 실제로 써먹을 일이 있었나요?</li>
  </ul>
</div>
...
나에게 익숙한 것에 착 달라붙어 빠져나오지 못하는 현상을 심리학에서는 '고착'이라 불러요. 이에 대해서는 [고착에서 빠져나와 창의적 통찰을 얻는 법](https://twinstae.github.io/fixation-to-insight/)이라는 글을 썼으니, 읽어보세요!

<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>내가 중요하지 않다 생각한 것 중에, 한 번 찍먹해볼만한 걸 찾아서 탐험해보세요.</li>
    <li>몇 년 전의 나와 지금의 나를 비교했을 때, 크게 변했다고 생각하나요? 업계의 기술 트렌드는 어떤가요?</li>
  </ul>
</div>
...
<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>남을 도와주는 건 손해보는 일일까요? 나를 희생하지 않으면서 가치 있는 일을 할 수는 없을까요?</li>
    <li>몇 년 전의 나와 지금의 나를 비교했을 때, 크게 변했다고 생각하나요? 업계의 기술 트렌드는 어떤가요?</li>
  </ul>
</div>
...
<div class="admonition note">
  <p class="admonition-title">✏️ 생각해보기</p>
  <ul>
    <li>돌이켜보세요. 스스로 몰아붙일수록 빠르게 성장했었나요?</li>
    <li>앞으로 초조하고 불안한 마음이 들 때, 어떻게 해야 더 잘 알아차릴 수 있을까요?</li>
  </ul>
</div>
...
[링크](https://www.codesoom.com/how-to-solve-it)
...
[링크](https://overreacted.io/ko/things-i-dont-know-as-of-2018/)
댄은 리액트 코어 팀의 프로그래머로, 리덕스도 만들었습니다. 이 글에서는 댄이 자기가 모르는 기술들에 대해 이야기합니다. 그는 재미있고 겸손한 사람이에요.

댄은 [가상 코딩 인터뷰를 하는 유튜브](https://www.youtube.com/watch?v=XEt09iK8IXs)에 출연한 적이 있습니다. 다음은 한 댓글이에요.

> the creator of redux taking so long to center a div gives me so much hope in my skills

### 삶을 충만하게 만드는 "지속 가능한" 운동 - 멜라니 조이
[링크](https://youtu.be/mXVyT7CzCT8)
멜라니 조이는 심리학을 전공한 동물권 운동가입니다. 이 영상은 동물권에 관심이 없더라도 보시면 좋다고 생각해요. 나를 희생하지 않으면서도 가치 있는 삶에서 에너지를 얻으며 살아가는 방법을 이야기합니다.